...
class NASNetwork(nn.Module):
  ...
  def __init__(self, C, num_classes, layers, criterion, device, n_channels=3, steps=4, multiplier=4, stem_multiplier=3, softmax=False):
    ...
    self.stem = nn.Sequential(
      nn.Conv2d(n_channels, C_curr, 3, padding=1, bias=False), #####
  ...
  def new(self):
    model_new = NASNetwork(self._C, self._num_classes, self._layers, self._criterion).to(self.device)#.cuda()
  ...
  def forward(self, input):
    ...
    if self.softmax: logits = F.softmax(logits)
    return logits
  ...
  def _initialize_alphas(self):
    ...
    self.alphas_normal = Variable(1e-3*torch.randn(k, num_ops).to(self.device), requires_grad=True) ###.cuda()
    self.alphas_reduce = Variable(1e-3*torch.randn(k, num_ops).to(self.device), requires_grad=True)
    self.betas_normal = Variable(1e-3*torch.randn(k).to(self.device), requires_grad=True)
    self.betas_reduce = Variable(1e-3*torch.randn(k).to(self.device), requires_grad=True)
